...
$path[] = "v1.0";
    ...
    function list_cdn_containers()
    {
        ...
        if ($return_code == 401) {
            return array($return_code,"Unauthorized",[]);
        }
        if ($return_code == 404) {
            return array($return_code,"Account not found.",[]);
        }
        if ($return_code == 204) {
...
$this->create_[];
    ...
    function list_containers($limit=0, $marker=NULL)
    {
        ...
        if ($return_code == 204) {
            return array($return_code, "Account has no containers.", []);
        }
        if ($return_code == 404) {
...
$this->create_[];
    ...
    #
    function get_object_to_string(&$obj, $hdrs=[])
    {
    ...
    # GET /v1/Account/Container/Object
    #
    function get_object_to_stream(&$obj, &$resource=NULL, $hdrs=[])
    {
        if (!is_object($obj) || get_class($obj) != "CF_Object") {
            throw new SyntaxException(
                "Method argument is not a valid CF_Object.");
