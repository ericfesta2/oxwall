...
class UTIL_Url
{
    ...
    {
        $redirectCode = array_key_exists((int) $redirectCode, self::REDIRECT_CODES) ? (int) $redirectCode : 301;

        header(self::REDIRECT_CODES[$redirectCode]);
        header("Location: " . $url);
        exit();
    }
    ...
    * @return string
    */
    public static function secureUri( string $uri ): string
    {
    ...
    public static function selfUrl(): string
    {
        ...
        $serverPort = $_SERVER['SERVER_PORT'] !== self::DEFAULT_PORT ? ":{$_SERVER['SERVER_PORT']}" : '';
        $protocol = substr($serverProtocol, 0, strpos($serverProtocol, '/')) . $s;

        return $protocol . '://' . $_SERVER['SERVER_NAME'] . $serverPort . self::secureUri($_SERVER['REQUEST_URI']);
    }
