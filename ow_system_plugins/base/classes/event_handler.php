...
class BASE_CLASS_EventHandler
{
    public function genericInit()
    {
        ...
        $eventManager->bind('base.splash_screen_exceptions', [$this, 'onAddSplashScreenExceptions']);
        ...
        $eventManager->bind('base.on_after_generate_email_verification_code', [$this, 'generateEmailVerificationCode']);
    }
...
//    }
    ...
    public function onAddMaintenanceModeExceptions(BASE_CLASS_EventCollector $event)
    {
        ...
        $event->add(['controller' => 'BASE_CTRL_Base', 'action' => 'sitemap']);
    }
    ...
    public function onProvideMetaInfoForPage(OW_Event $event)
    {
        ...
        if (!empty($params['title'])) {
            ...
            if (!empty($parts[1]) && $parts[1] === 'meta_title_user_page') {
                if (empty($vars['user_gender']) || empty($vars['user_age'])
