...
class BOL_TagDao extends OW_BaseDao
{
    ...
    public function findTagListByEntityIdList( $entityType, array $idList ): array
    {
        $query = "
            SELECT `t`.`label`, `et`.*  FROM " . $this->getTableName() . " AS `t`
			INNER JOIN `" . BOL_EntityTagDao::getInstance()->getTableName() . "` AS `et`
            ON `et`.`tagId` = `t`.`id`
     ...
     * @return array
     */
    public function findMostPopularTags( $entityType, int $limit, int $offset = 0)
    {
        ...
        return $this->dbo->queryForList($query, [
            'offset' => (int) $offset, 'limit' => (int) $limit, 'entityType' => $entityType
        ]);
    }
    ...
    public function findEntityTagsWithPopularity( $entityId, $entityType )
    {
        ...
        $query = "
            SELECT * FROM (
                ...
                AND " . $queryParts['where'] . "
                GROUP BY `et`.`id`, `tagId` ORDER BY `count` DESC
            ) AS `t` 
            ORDER BY `t`.`label`";
